...
docker run -d --net="host" -v $PWD:/var/www/webapp:rw --name simplecases-rails-prod claudevandort/simplecases-rails:prod /bin/bash -l -c "whenever --update-crontab; /usr/sbin/apache2 -D FOREGROUND; chmod -R 777 tmp/*"
