...
app.controller "GetCausesController", ['$scope', '$http', ($scope, $http) ->
	$http.get(window.location.hostname + '/causes.json').success (data) ->
		console.log data
		$scope.causes = data
	$http.get(window.location.hostname + '/clients.json').success (data) ->
		console.log data
		$scope.clients = data
]
app.controller "GetClientController", ['$scope', '$http', ($scope, $http) ->
	$scope.getClient = -> 
		...
		$http.get(window.location.hostname + '/clients.json?id_number=' + rut).success (data, status) ->
...
app.controller "CourtController", ['$scope', '$http', ($scope, $http) ->
	...
	$http.get(window.location.hostname + '/courts.json?area_id=' + angular.element('#cause_area_id').val()).success (data, status) ->
		$scope.courts = data
	$scope.getCourts = ->
		$http.get(window.location.hostname + '/courts.json?area_id=' + angular.element('#cause_area_id').val()).success (data, status) ->
			$scope.courts = data
